 ...
 * This version is modified to output classes instead of inline styles.
 */
export default function markedAlert(options: AlertOptions = {}): MarkedExtension {
  const { className = `mpe-alert`, variants = [] } = options
  const resolvedVariants = resolveVariants(variants)

  function buildMeta(variantType: string, matchedVariant: AlertVariantItem) {
    return {
      className,
      variant: variantType,
  ...
  function renderAlert(token: any) {
    ...
    // Add classes to the content paragraphs
    text = text.replace(/<p>/g, `<p class="${meta.className}-content">`)

    let tmpl = `<blockquote class="${meta.className} ${meta.className}-${meta.variant}">
`
    tmpl += `<p class="${meta.titleClassName} ${meta.titleClassName}-${meta.variant}">`
    tmpl += meta.icon
    tmpl += meta.title
    tmpl += `</p>
`
  ...
  return {
    ...
    walkTokens(token) {
      ...
      const matchedVariant = resolvedVariants.find(({ type }) =>
        new RegExp(createSyntaxPattern(type), `i`).test((token as any).text),
      )
