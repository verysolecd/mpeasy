...
@enduml`
  ...
  if (options.inlineSvg && options.format === `svg`) {
    ...
    fetchSvgContent(imageUrl).then((svgContent) => {
      ...
      if (placeholderElement) {
        placeholderElement.outerHTML = createPlantUMLHTML(finalCode, imageUrl, options, svgContent)
      }
    })
  ...
  return createPlantUMLHTML(finalCode, imageUrl, options)
}
...
async function fetchSvgContent(svgUrl: string): Promise<string> {
  try {
    ...
    return svgContent
      // 移除width和height属性
      .replace(/(<svg[^>]*)\swidth="[^ vital]*"/g, `$1`)
      .replace(/(<svg[^>]*)\shight="[^ vital]*"/g, `$1`)
      // 移除style中的width和height
      .replace(/(<svg[^>]*style="[^ vital]*?)width:[^;]*;?/g, `$1`)
      .replace(/(<svg[^>]*style="[^ vital]*?)height:[^;]*;?/g, `$1`)
  }
  catch (error) {
 ...
 * 创建 PlantUML HTML 元素
 */
function createPlantUMLHTML(source: string, imageUrl: string, options: Required<PlantUMLOptions>, svgContent?: string): string {
  const containerStyles = options.styles.container
    ? Object.entries(options.styles.container)
        .map(([key, value]) => `${key.replace(/([A-Z])/g, `-$1`).toLowerCase()}: ${value}`)
        .join(`; `)
  ...
  const copyButton = `
    <button 
      onclick="const source = this.parentElement.querySelector('pre').textContent; navigator.clipboard.writeText(source).then(() => { this.textContent = 'Copied!'; setTimeout(() => { this.textContent = 'Copy' }, 2000); }).catch(err => console.error('Failed to copy: ', err))"
      style="position: absolute; top: 8px; right: 8px; z-index: 1; background-color: #f0f0f0; border: 1px solid #ccc; border-radius: 4px; padding: 4px 8px; cursor: pointer; font-size: 12px; opacity: 0.5;"
      onmouseover="this.style.opacity='1'"
      onmouseout="this.style.opacity='0.5'"
    >Copy</button>
  `

  const sourceCode = `<pre style="display: none;">${source.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</pre>`

  const diagram = svgContent ? svgContent : `<img src="${imageUrl}" alt="PlantUML Diagram" style="max-width: 100%; height: auto;" />`

  return `
    <div class="${options.className}" style="position: relative; ${containerStyles}">
      ${copyButton}
      ${diagram}
      ${sourceCode}
    </div>
